...
namespace {
PlatformTaskRunner g_platform_task_runner;
}  // namespace

void SetPlatformTaskRunner(PlatformTaskRunner runner) {
  g_platform_task_runner = runner;
}

PlatformTaskRunner GetPlatformTaskRunner() {
  return g_platform_task_runner;
}
 ...
 public:
  EventChannelProxyImpl(BinaryMessenger* messenger,
    ...
    auto handler = std::make_unique<
        ...
        [&](const EncodableValue* arguments,
          ...
          sink_ = std::shared_ptr<EventSink>(std::move(events));
  ...
  void Success(const EncodableValue& event, bool cache_event = true) override {
    if (on_listen_called_) {
      auto runner = GetPlatformTaskRunner();
      auto event_copy = event;
      auto weak_sink = std::weak_ptr<EventSink>(sink_);
      auto send = [weak_sink, event_copy]() {
        auto sink = weak_sink.lock();
        if (sink) {
          sink->Success(event_copy);
        }
      };
      if (runner) {
        runner(std::move(send));
      } else {
        send();
      }
    } else {
