...
#   ./run_tests.sh station    # Run station API tests only
#   ./run_tests.sh tiles    # Run tile server tests (standalone)
#   ./run_tests.sh all      # Run all tests (station + tiles)
...
run_flutter_tests() {
    ...
    echo "      Run './run_tests.sh station' for actual API tests"
}
...
case "${1:-station}" in
    ...
    help|--help|-h)
        ...
        echo ""
        echo "Commands:"
        echo "  station    Run station API tests (standalone) [default]"
        ...
        echo "  all        Run station + tile + alert + alert-api tests"
        echo "  help       Show this help message"
        echo ""
        echo "Alert tests verify:"
        echo "  - NOSTR event creation and signing"
        echo "  - WebSocket communication with station"
        echo "  - Alert event verification (BIP-340 Schnorr)"
        echo "  - Alert storage on station disk"
